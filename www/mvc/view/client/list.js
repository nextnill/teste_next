function listar()
{
    ...
    $.getJSON("<?= APP_URI ?>client/list/json/", function(response) {
...
function add_row(table_body, item)
{
    ...
    var field_head_office_name = $(new_row.find("[template-field='head_office_name']"));
    field_head_office_name.text(item.head_office_name ? item.head_office_name : '');
