function listar()
{
    ...
    $.getJSON("<?= APP_URI ?>user/list/json/", function(response) {
...
function add_row(table_body, item)
{
    ...
    var field_blocked = $(new_row.find("[template-field='blocked']"));
    field_blocked.html(str_yes_no(item.blocked));

    var button_permission = new_row.find("[template-button='permission']");
    button_permission.attr('template-ref', item.id);
    button_permission.click(function() {
