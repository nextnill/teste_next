...
function per_list_client_groups(selected_values, readonly)
{
    ...
    $.getJSON("<?= APP_URI ?>user/detail/json/" + user_id, function(response) {
        if (response_validation(response)) {
            ...
            if (response.hasOwnProperty('id'))
            {
                ...
                per_list_client_groups(response.client_groups, response.admin == true);
                per_compare();
            }
        }
    }).fail(ajaxError);
    ...
    if (per_valida_formulario())
    {
        ...
        $.ajax({
            ...
            data: {
                ...
                client_group: cbo_client_groups.val(),
